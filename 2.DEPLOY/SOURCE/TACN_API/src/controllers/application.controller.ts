...
import { IS_ACTIVE, apiCode, CONFIG } from "@utils/constant";
...
class ApplicationController {
  ...
  async _findAndCountAll(
    options: Object = {},
    page: number,
    model: string = ""
  ) {
    const { count, rows } = await db[model || this.model].findAndCountAll(
      options
    );
  ...
  async _findAll(
    ...
    const rows = await db[model || this.model].findAll(
      options
    );
